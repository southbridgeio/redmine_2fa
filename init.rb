...
ActionDispatch::Callbacks.to_prepare do
  ...
  paths = '/app/models/redmine_2fa/auth_source/*.rb'
  Dir.glob(File.dirname(__FILE__) + paths).each do |file|
    require_dependency file
  end

  paths = '/lib/redmine_2fa/patches/account_controller_patch/*.rb'
  Dir.glob(File.dirname(__FILE__) + paths).each do |file|
    require_dependency file
  end
...
Redmine::Plugin.register :redmine_2fa do
  ...
  settings(default: { 'bot_token' => 'bot_token', 'required' => true },
